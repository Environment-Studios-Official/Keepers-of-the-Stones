	...
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (PowerModVariables.MapVariables.get(world).orange_portal) {
			if (!PowerModVariables.MapVariables.get(world).blue_portal) {
				world.setBlock(BlockPos.containing(x, y + 1, z), PowerModBlocks.BLUE_PORTAL.get().defaultBlockState(), 3);
			...
			if (!PowerModVariables.MapVariables.get(world).orange_portal) {
				world.setBlock(BlockPos.containing(x, y + 1, z), PowerModBlocks.ORANGE_PORTAL.get().defaultBlockState(), 3);
