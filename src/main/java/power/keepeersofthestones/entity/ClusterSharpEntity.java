...
public class ClusterSharpEntity extends AbstractArrow implements ItemSupplier {
	public ClusterSharpEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(PowerModEntities.CLUSTER_SHARP.get(), world);
	}
	...
	public ItemStack getItem() {
		return new ItemStack(PowerModItems.CLUSTER_SHARP.get());
	}
	...
	public static ClusterSharpEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		ClusterSharpEntity entityarrow = new ClusterSharpEntity(PowerModEntities.CLUSTER_SHARP.get(), entity, world);
	...
	public static ClusterSharpEntity shoot(LivingEntity entity, LivingEntity target) {
		ClusterSharpEntity entityarrow = new ClusterSharpEntity(PowerModEntities.CLUSTER_SHARP.get(), entity, entity.level);
		...
		entity.level.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.amethyst_block.break")), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
